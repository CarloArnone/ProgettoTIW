...
/*!50001 VIEW `asteaperte` (`idAsta`,`prezzoMassimoRaggiunto`,`rialzoMinimo`,`dataScadenza`,`oreRimanenti`) AS select `a`.`id` AS `id`,0 AS `0`,`a`.`rialzoMinimo` AS `rialzoMinimo`,`a`.`dataTermine` AS `dataTermine`,timestampdiff(HOUR,`a`.`dataTermine`,`u`.`lastLogin`) AS `TIMESTAMPDIFF(HOUR, A.dataTermine, U.lastLogin )` from (`aste` `a` join `utenti` `u` on((`a`.`idCreatore` = `u`.`id`))) where ((`a`.`closed` = false) and `a`.`id` in (select distinct `offerte`.`idAsta` from `offerte`) is false) union select `a`.`id` AS `id`,`o`.`prezzoOfferto` AS `prezzoOfferto`,`a`.`rialzoMinimo` AS `rialzoMinimo`,`a`.`dataTermine` AS `dataTermine`,timestampdiff(HOUR,`a`.`dataTermine`,`u`.`lastLogin`) AS `TIMESTAMPDIFF(HOUR, A.dataTermine, U.lastLogin )` from ((`aste` `a` join `offerte` `o` on((`a`.`id` = `o`.`idAsta`))) left join `utenti` `u` on((`u`.`id` = `o`.`idCreatore`))) where ((`a`.`closed` = false) and (`o`.`prezzoOfferto` = (select max(`o1`.`prezzoOfferto`) from `offerte` `o1` where (`a`.`id` = `o1`.`idAsta`)))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`carlo`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `astechiuse` (`idAsta`,`idVincitore`,`prezzoFinale`,`indSpedizione`) AS select `a`.`id` AS `id`,`o`.`idCreatore` AS `idCreatore`,`o`.`prezzoOfferto` AS `prezzoOfferto`,`u`.`indirizzoSpedizione` AS `indirizzoSpedizione` from ((`aste` `a` left join `offerte` `o` on((`a`.`id` = `o`.`idAsta`))) left join `utenti` `u` on((`o`.`idCreatore` = `u`.`id`))) where ((`a`.`closed` = true) and ((`o`.`prezzoOfferto` = (select max(`offerte`.`prezzoOfferto`) from `offerte` where (`a`.`id` = `offerte`.`idAsta`))) or (`o`.`prezzoOfferto` is null))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2023-05-05 12:59:10
